...
function getDepPath(dep) {
    return findRoot(require.resolve(dep));
}
...
module.exports = (env, argv) => {
    ...
    const jsSdkSrcDir = path.resolve(getDepPath("matrix-js-sdk"), "src");
    ...
    return {
        ...
        resolve: {
            ...
            alias: {
                ...
                "react": getDepPath("react"),
                "react-dom": getDepPath("react-dom"),

                // Same goes for js - we don't need two copies.
                "matrix-js-sdk": getDepPath("matrix-js-sdk"),
                "@matrix-org/react-sdk-module-api": getDepPath("@matrix-org/react-sdk-module-api"),
                // and matrix-widget-api
                "matrix-widget-api": getDepPath("matrix-widget-api"),
                "oidc-client-ts": getDepPath("oidc-client-ts"),

                // Define a variable so the i18n stuff can load
                "$webapp": path.resolve(__dirname, "webapp"),

                // Make shared-components imports resolve to EW deps
                "counterpart": getDepPath("counterpart"),
                "@vector-im/compound-web": getDepPath("@vector-im/compound-web"),
            },
            fallback: {
