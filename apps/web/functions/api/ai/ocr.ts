...
export const onRequestPost = async ({ request, env }: Context): Promise<Response> => {
    if (!env.SPARK_API_KEY || !env.SPARK_BASE_URL || !env.SPARK_OCR_MODEL) return jsonError("AI 服务尚未完成部署配置", 503);
    const body = (await request.json().catch(() => undefined)) as { image?: unknown } | undefined;
    if (typeof body?.image !== "string" || !body.image.startsWith("data:image/")) return jsonError("缺少可识别的图片数据");
            ...
            messages: [
                {
                    role: "user",
                    content: [
                        { type: "text", text: "识别图片中的全部文字，只返回清理后的纯文本，不要 Markdown、HTML、坐标或解释。" },
