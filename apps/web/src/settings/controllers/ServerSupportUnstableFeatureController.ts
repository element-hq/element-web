...
export default class ServerSupportUnstableFeatureController extends MatrixClientBackedController {
    ...
    public set disabled(newDisabledValue: boolean) {
        ...
        const level = this.sdkContext.settingsStore.firstSupportedLevel(this.settingName);
        if (!level) return;
        const settingValue = this.sdkContext.settingsStore.getValue(this.settingName, null);
