...
import { render, screen, cleanup, fireEvent, waitFor } from "test-utils-rtl";
import {
    Room,
    RoomStateEvent,
    type MatrixEvent,
...
describe("IncomingCallToast", () => {
    ...
    beforeEach(async () => {
        ...
        vi.spyOn(ToastStore, "sharedInstance").mockReturnValue(toastStore);
    ...
    it.each(["video", "voice"])("shows information for a group %s call", (callType: string) => {
        ...
        expect(screen.getByText("Group call started")).toBeVisible();
        ...
        expect(screen.getByLabelText("@bob:example.org")).toBeVisible();
        expect(screen.getByText("on the call")).toBeVisible();
    ...
    it.each(["video", "voice"])("shows information for a DM %s call", (callType: string) => {
        vi.mocked(dmRoomMap.getUserIdForRoomId).mockImplementation((roomId) =>
    ...
    it("start ringing on ring notify event", () => {
        ...
        const playMock = vi.spyOn(sdkContext.legacyCallHandler, "play");
        render(
