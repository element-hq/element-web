...
export const transformTags = {
    ...
    "a": function (tagName: string, attribs: HtmlSanitizeAttributes) {
        if (attribs.href) {
    ...
    "img": function (tagName: string, attribs: HtmlSanitizeAttributes) {
        let src = attribs.src;
    ...
    "code": function (tagName: string, attribs: HtmlSanitizeAttributes) {
    ...
    "*": function (tagName: string, attribs: HtmlSanitizeAttributes) {
...
} satisfies NonNullable<HtmlSanitizeOptions["transformTags"]>;

// The shared package owns the Element Web baseline policy. These transforms
// only add browser/app context which cannot be represented by a string-only
// sanitizer: MXC thumbnail URLs and local permalink handling.
export const sanitizeHtmlParams: HtmlSanitizeOptions = {
    transformTags,
};
...
export function linkifyAndSanitizeHtml(
    ...
    return sanitizeHtmlString(linkifyString(dirtyHtml, options), sanitizeHtmlParams);
}
