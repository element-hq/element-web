...
describe("/part", () => {
    ...
    function setUp(): {
        ...
        vi.mocked(client.getRooms).mockReturnValue([room1, room2]);
    ...
    it("should part room matching alias if found", async () => {
        ...
        room1.getCanonicalAlias = vi.fn().mockReturnValue("#foo:bar");
        room2.getCanonicalAlias = vi.fn().mockReturnValue("#baz:bar");
    ...
    it("should part room matching alt alias if found", async () => {
        const { client, command, args, room1, room2 } = setUp();
        room1.getAltAliases = vi.fn().mockReturnValue(["#foo:bar"]);
        room2.getAltAliases = vi.fn().mockReturnValue(["#baz:bar"]);
