...
import { ElementWidget, WidgetMessaging } from "./WidgetMessaging";
...
import { ElementWidgetActions, type IViewRoomApiRequest } from "./ElementWidgetActions";
...
describe("WidgetMessaging", () => {
    ...
    beforeEach(() => {
        ...
        messaging = vi.mocked(last(vi.mocked(ClientWidgetApi).mock.instances)!);
    ...
    describe("feed event", () => {
        ...
        it("feeds decrypted events asynchronously", async () => {
            ...
            const decryptingSpy1 = vi.spyOn(event1Encrypted, "isBeingDecrypted").mockReturnValue(true);
            ...
            const decryptingSpy2 = vi.spyOn(event2Encrypted, "isBeingDecrypted").mockReturnValue(true);
...
describe("WidgetMessaging with stickyPromise", () => {
    ...
    it("should wait for the sticky promise to resolve before starting messaging", async () => {
        ...
        const setPersistenceSpy = vi.spyOn(ActiveWidgetStore.instance, "setWidgetPersistence");
        ...
        await waitFor(() => expect(setPersistenceSpy).toHaveBeenCalled(), { interval: 5 });
    });
});
...
describe("WidgetMessaging as an account widget", () => {
    ...
    beforeEach(() => {
        ...
        getRoomId = vi.spyOn(SDKContextClass.instance.roomViewStore, "getRoomId") as unknown as MockedFunction<
            () => string | null
        >;
...
describe("WidgetMessaging action handling", () => {
    ...
    beforeEach(() => {
        ...
        widget.start(null as unknown as HTMLIFrameElement);
        messaging = vi.mocked(last(vi.mocked(ClientWidgetApi).mock.instances)!);
        Object.defineProperty(messaging, "transport", { value: { reply: vi.fn() } });
    ...
    describe("open modal widget", () => {
        beforeEach(() => {
            ...
            let hasModal = false;
            vi.spyOn(ModalWidgetStore.instance, "openModalWidget").mockImplementation(() => {
                if (hasModal) {
                    throw Error("Modal already in view");
                }
                hasModal = true;
            });
            vi.spyOn(ModalWidgetStore.instance, "canOpenModalWidget").mockImplementation(() => !hasModal);
            vi.spyOn(ModalWidgetStore.instance, "closeModalWidget").mockImplementation(() => {
                if (!hasModal) {
        ...
        it("responds with an error if a modal is already open", () => {
            ...
            actionFns[`action:${WidgetApiFromWidgetAction.OpenModalWidget}`](ev);
            expect(messaging.transport.reply).toHaveBeenCalledWith(ev.detail, {});
            vi.mocked(messaging.transport.reply).mockReset();
            actionFns[`action:${WidgetApiFromWidgetAction.OpenModalWidget}`](ev);
            expect(messaging.transport.reply).toHaveBeenCalledWith(ev.detail, {
    ...
    describe("view room", () => {
        ...
        it("handles room change", async () => {
            ...
            const dispatch = (defaultDispatcher.dispatch = vi.fn());
            actionFns[`action:${ElementWidgetActions.ViewRoom}`](ev);
            expect(messaging.transport.reply).toHaveBeenCalledWith(ev.detail, {});
    ...
    describe("always on screen", () => {
        let setWidgetPersistence: Mock<ActiveWidgetStore["setWidgetPersistence"]>;
        beforeEach(() => {
            setWidgetPersistence = vi.spyOn(ActiveWidgetStore.instance, "setWidgetPersistence");
        });
...
describe("WidgetMessaging action handling for stickerpicker", () => {
    ...
    beforeEach(() => {
        ...
        const getRoomId = vi.spyOn(SDKContextClass.instance.roomViewStore, "getRoomId") as unknown as MockedFunction<
            () => string | null
        >;
    ...
    describe("open integrations manager", () => {
        let openIntegrationManager: Mock<IntegrationManagerInstance["open"]>;
        beforeEach(() => {
            // Trivial mock of ModalWidgetStore
            openIntegrationManager = vi.fn();
            const inst = IntegrationManagers.sharedInstance();
            vi.spyOn(inst, "getPrimaryManager").mockImplementation(
                () =>
                    ({
