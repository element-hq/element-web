...
import { vi, describe, it, expect, beforeEach, type Mock, type MockedObject } from "vitest";
...
describe("ElementWidgetDriver", () => {
    ...
    it("approves capabilities via module api", async () => {
        ...
        vi.spyOn(ModuleRunner.instance, "invoke").mockImplementation((lifecycleEvent, opts, widgetInfo, requested) => {
    ...
    it("approves capabilities via new widget lifecycle API", async () => {
        ...
        vi.spyOn(ModuleApi.instance.widgetLifecycle, "preapproveCapabilities").mockResolvedValue(requestedCapabilities);
    ...
    it("falls back to legacy module API when new API returns undefined for capabilities", async () => {
        ...
        vi.spyOn(ModuleApi.instance.widgetLifecycle, "preapproveCapabilities").mockResolvedValue(undefined);
        vi.spyOn(ModuleRunner.instance, "invoke").mockImplementation((lifecycleEvent, opts, widgetInfo, requested) => {
            if (lifecycleEvent === WidgetLifecycle.CapabilitiesRequest) {
                (opts as CapabilitiesOpts).approvedCapabilities = requested;
            }
    ...
    it("approves identity via new widget lifecycle API", async () => {
        ...
        vi.spyOn(ModuleApi.instance.widgetLifecycle, "preapproveIdentity").mockResolvedValue(true);
    ...
    it("falls back to legacy module API when new API returns false for identity", async () => {
        ...
        vi.spyOn(ModuleApi.instance.widgetLifecycle, "preapproveIdentity").mockResolvedValue(false);
        vi.spyOn(ModuleRunner.instance, "invoke").mockImplementation((lifecycleEvent, opts, widgetInfo) => {
            if (lifecycleEvent === WidgetLifecycle.IdentityRequest) {
                (opts as ApprovalOpts).approved = true;
            }
    ...
    describe("getTurnServers", () => {
        ...
        it("stops if VoIP isn't supported", async () => {
            vi.spyOn(client, "pollingTurnServers", "get").mockReturnValue(false);
    ...
    describe("getRtcTransports", () => {
        ...
        it("gets the RTC transports from the homeserver", async () => {
            ...
            vi.mocked(client.cachedRtcTransports.wait).mockResolvedValue(transports);
        ...
        it("propagates errors from the homeserver", async () => {
            ...
            vi.mocked(client.cachedRtcTransports.wait).mockRejectedValue(error);
        ...
        it("Should not fallback to well-known if config disallows and transport discovery not available", async () => {
            ...
            vi.mocked(client.cachedRtcTransports.wait).mockRejectedValue(
        ...
        it("Should not fallback to well-known if config disallows and homerserver advertise no transports", async () => {
            ...
            vi.mocked(client.cachedRtcTransports.wait).mockResolvedValue([]);
    ...
    describe("readEventRelations", () => {
        ...
        it("reads related events from the current room", async () => {
            vi.spyOn(SDKContextClass.instance.roomViewStore, "getRoomId").mockReturnValue("!this-room-id");
    ...
    describe("chat effects", () => {
        ...
        beforeEach(() => {
            stubClient();
            driver = mkDefaultDriver();
            vi.spyOn(dis, "dispatch").mockReset();
        });
    ...
    describe("StickyEvent", () => {
        ...
        it("Reads sticky message", async () => {
            ...
            const mockRoom: Partial<Room> = {
                roomId: roomId,
                _unstable_getStickyEvents: vi.fn().mockReturnValue([mockStickyEvent]),
            };
