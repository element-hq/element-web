...
describe("WidgetLayoutStore", () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(WidgetStore, "instance", "get").mockReturnValue({
            on: vi.fn(),
            off: vi.fn(),
            getApps: () => mockApps,
    ...
    it("should recalculate all rooms when the client is ready", async () => {
        vi.mocked(client.getVisibleRooms).mockReturnValue([mockRoom]);
        await store.start();
    ...
    it("should clear the layout and emit an update if there are no longer apps in the room", () => {
        store.recalculateRoom(mockRoom);
        vi.mocked(roomUpdateListener).mockClear();

        vi.spyOn(WidgetStore, "instance", "get").mockReturnValue(<WidgetStore>(
    ...
    it("should copy the layout to the room", async () => {
        ...
        expect(vi.mocked(client.sendStateEvent).mock.calls).toMatchInlineSnapshot(`
            [
              [
                "!room:server",
    ...
    it("Can call onNotReady before onReady has been called", () => {
        ...
        expect(() => store.onNotReady()).not.toThrow();
    });
    ...
    describe("when feature_dynamic_room_predecessors is not enabled", () => {
        ...
        it("passes the flag in to getVisibleRooms", async () => {
            vi.mocked(client.getVisibleRooms).mockRestore();
