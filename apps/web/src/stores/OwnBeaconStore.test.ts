...
import { describe, it, expect, beforeEach, afterEach, afterAll, vi, type Mocked } from "vitest";
import {
    advanceDateAndTime,
...
import { OwnBeaconStore, OwnBeaconStoreEvent } from "./OwnBeaconStore";
...
vi.mock("lodash", async () => ({
    ...
    debounce: vi.fn().mockImplementation((callback) => callback),
}));
...
describe("OwnBeaconStore", () => {
    ...
    vi.spyOn(global.Date, "now").mockReturnValue(now - HOUR_MS);
    ...
    const localStorageGetSpy = vi.spyOn(localStorage, "getItem").mockReturnValue(null);
    const localStorageSetSpy = vi.spyOn(localStorage, "setItem").mockImplementation(() => {});
    ...
    beforeEach(() => {
        ...
        vi.spyOn(OwnBeaconStore.instance, "emit").mockRestore();
    ...
    describe("onNotReady()", () => {
        it("removes listeners", async () => {
            const store = await makeOwnBeaconStore();
            const removeSpy = vi.spyOn(mockClient, "removeListener");
            // @ts-ignore
            store.onNotReady();
        ...
        it("destroys beacons", async () => {
            ...
            const destroySpy = vi.spyOn(beacon, "destroy");
            // @ts-ignore
            store.onNotReady();
    ...
    describe("on new beacon event", () => {
        ...
        it("ignores events for irrelevant beacons", async () => {
            ...
            const monitorSpy = vi.spyOn(bobsLiveBeacon, "monitorLiveness");
        ...
        it("adds users beacons to state and monitors liveness", async () => {
            ...
            const monitorSpy = vi.spyOn(alicesLiveBeacon, "monitorLiveness");
    ...
    describe("on room membership changes", () => {
        ...
        it("destroys and removes beacons when current user leaves room", async () => {
            ...
            const beaconDestroySpy = vi.spyOn(room1BeaconInstance, "destroy");
            const emitSpy = vi.spyOn(store, "emit");
    ...
    describe("publishing positions", () => {
        ...
        describe("when store is initialised with live beacons", () => {
            ...
            it("kills live beacon when geolocation is unavailable", async () => {
                ...
                vi.spyOn(navigator, "geolocation", "get").mockReturnValue(undefined as unknown as Geolocation);
