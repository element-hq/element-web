...
import { SetupEncryptionStore } from "./SetupEncryptionStore";
...
describe("SetupEncryptionStore", () => {
    ...
    describe("usePassPhrase", () => {
        it("should use dehydration when enabled", async () => {
            ...
            vi.mocked(accessSecretStorage).mockImplementation(async (func?: () => Promise<void>) => {
                await func!();
            });
            ...
            await expect(dehydrationPromise).resolves.toBeUndefined();
        });
    });
});
