...
import { E2eMessageSharedIconViewModel } from "./E2eMessageSharedIconViewModel";

type TestRoomState = EventEmitter & {
    getMember: Mock<(userId: string) => RoomMember | null>;
};
...
): Mock<(userId: string) => RoomMember | null> {
    const getMember = vi.fn((userId: string): RoomMember | null => {
...
function createRoom(roomState: RoomState): Room {
    const room = {
        getLiveTimeline: vi.fn(() => ({
            getState: vi.fn((timeline: EventTimeline) => {
