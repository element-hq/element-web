...
const clientMxcUrlToHttpMocks = new WeakMap<MatrixClient, Mock>();
...
function makeClient(decryptEventIfNeeded = vi.fn().mockResolvedValue(undefined)): MatrixClient {
...
function getMxcUrlToHttpMock(cli: MatrixClient): Mock {
...
function makeMember(name: string, senderUserId = userId, mxcAvatarUrl?: string): RoomMember {
    return {
        ...
        roomId,
        getAvatarUrl: vi.fn(),
        getMxcAvatarUrl: vi.fn(() => mxcAvatarUrl),
...
describe("ThreadSummaryViewModel", () => {
    beforeEach(() => {
        ...
        vi.spyOn(MessagePreviewStore.instance, "generatePreviewForEvent").mockReturnValue("Latest reply");
    });
    ...
    it("does not emit when setters receive unchanged values", () => {
        ...
        const setClientSpy = vi.spyOn(previewVm, "setClient");
        const setRoomSpy = vi.spyOn(previewVm, "setRoom");
        const setTimelineRenderingTypeSpy = vi.spyOn(previewVm, "setTimelineRenderingType");
        const setLowBandwidthSpy = vi.spyOn(previewVm, "setLowBandwidth");
        const setUseOnlyCurrentProfilesSpy = vi.spyOn(previewVm, "setUseOnlyCurrentProfiles");
...
describe("ThreadMessagePreviewViewModel", () => {
    beforeEach(() => {
        ...
        vi.spyOn(MessagePreviewStore.instance, "generatePreviewForEvent").mockImplementation((event) => {
    ...
    it("skips preview recomputation when guarded setters receive unchanged values", async () => {
        ...
        await waitFor(() => expect(vm.getSnapshot().isVisible).toBe(true));
        vi.mocked(cli.decryptEventIfNeeded).mockClear();
    ...
    it("updates when the watched event is replaced or decrypted", async () => {
        ...
        vi.mocked(MessagePreviewStore.instance.generatePreviewForEvent).mockReturnValue("Edited reply");
        event.emit(MatrixEventEvent.Replaced, event);

        await waitFor(() => expect(vm.getSnapshot().previewContent).toBe("Edited reply"));

        vi.mocked(MessagePreviewStore.instance.generatePreviewForEvent).mockReturnValue("Decrypted reply");
        event.emit(MatrixEventEvent.Decrypted, event);
    ...
    it("hides redacted replies and replies without generated previews", async () => {
        ...
        vi.spyOn(redactedEvent, "isRedacted").mockReturnValue(true);
        ...
        vi.mocked(MessagePreviewStore.instance.generatePreviewForEvent).mockReturnValue("");
    ...
    it("shows a decryption failure preview without trying to decrypt again", async () => {
        ...
        vi.spyOn(decryptionFailureEvent, "isDecryptionFailure").mockReturnValue(true);
    ...
    it("logs and hides the preview when decryption rejects", async () => {
        ...
        const cli = makeClient(vi.fn().mockRejectedValue(decryptError));
    ...
    it("catches fire-and-forget preview update errors", async () => {
        ...
        vi.mocked(MessagePreviewStore.instance.generatePreviewForEvent).mockImplementation(() => {
            throw previewError;
        });
        const loggerSpy = vi.spyOn(logger, "error").mockImplementation(() => {});
    ...
    it("shows a decryption failure preview when decryption rejects into a failure state", async () => {
        ...
        const isDecryptionFailureSpy = vi.spyOn(event, "isDecryptionFailure").mockReturnValue(false);
        const cli = makeClient(
    ...
    it("hides the preview when an event is redacted during decryption", async () => {
        ...
        const isRedactedSpy = vi.spyOn(event, "isRedacted").mockReturnValue(false);
        const cli = makeClient(
            vi.fn().mockImplementation(async () => {
    ...
    it("ignores stale decrypt results when the latest reply changes during decryption", async () => {
        ...
        const cli = makeClient(vi.fn().mockReturnValueOnce(firstDecrypt).mockResolvedValue(undefined));
        const room = new TestRoom() as TestRoom & Room;
        const thread = new TestThread(room) as TestThread & Thread;
        thread.replyToEvent = makeEvent("First reply");
        const vm = new ThreadMessagePreviewViewModel({
    ...
    it("falls back to the sender id when event sender profile data is missing", async () => {
        ...
        mxEvent.sender = null;
        vi.spyOn(mxEvent, "getSender").mockReturnValue("@fallback:example.org");
        const { vm } = makePreviewVm({
            thread: Object.assign(new TestThread(new TestRoom() as TestRoom & Room), {
