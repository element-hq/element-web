...
import { WidgetContextMenuViewModel, type WidgetContextMenuViewModelProps } from "./WidgetContextMenuViewModel";
...
import * as widgetStore from "../../../stores/WidgetStore";
...
describe("WidgetContextMenuViewModel", () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(WidgetUtils, "isManagedByManager").mockReturnValue(true);
        vi.spyOn(WidgetUtils, "editWidget").mockReturnValue();
        const mockMessaging = {
            on: () => {},
            off: () => {},
            stop: () => {},
        ...
        vi.spyOn(WidgetMessagingStore.instance, "getMessagingForUid").mockReturnValue(mockMessaging);
        client = stubClient();
    });
    ...
    it("should move widget position when onmovebutton is called", () => {
        vi.spyOn(WidgetLayoutStore.instance, "moveWithinContainer").mockReturnValue();
        const props = {
            ...defaultProps,
            room: new Room(roomId, client, userId),
        };
    ...
    it("should startJitsiAudioLivestream when onStreamAudioClick button is clicked", async () => {
        vi.spyOn(livestream, "startJitsiAudioLivestream").mockImplementation(vi.fn());
        vi.spyOn(livestream, "getConfigLivestreamUrl").mockReturnValue("https://url");
        const props = {
            ...defaultProps,
            room: new Room(roomId, client, userId),
        };
    ...
    it("should show modal when startJitsiAudioLivestream is on error and onStreamAudioClick button is clicked", async () => {
        vi.spyOn(livestream, "startJitsiAudioLivestream").mockImplementation(() => {
    ...
    it("should set new level for allowedwidget when onrevoke button is clicked", () => {
        ...
        vi.spyOn(SettingsStore, "getValue").mockReturnValue(current);
        vi.spyOn(SettingsStore, "firstSupportedLevel").mockReturnValue(SettingLevel.DEFAULT);
        vi.spyOn(SettingsStore, "setValue").mockResolvedValue();
        vi.spyOn(widgetStore, "isAppWidget").mockReturnValue(true);
        const props = {
            ...defaultProps,
            room: new Room(roomId, client, userId),
        };
        ...
        vi.spyOn(SettingsStore, "getValue").mockReturnValue(current2);
        vi.spyOn(SettingsStore, "firstSupportedLevel").mockReturnValue(SettingLevel.DEFAULT);
        vi.spyOn(SettingsStore, "setValue").mockResolvedValue();
        vi.spyOn(widgetStore, "isAppWidget").mockReturnValue(false);
    ...
    it("should throw an error when first supported level is not set", () => {
        vi.spyOn(SettingsStore, "firstSupportedLevel").mockReturnValue(null);
        const props = {
            ...defaultProps,
            room: undefined,
            onDeleteClick: undefined,
