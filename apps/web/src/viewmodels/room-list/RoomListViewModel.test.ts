...
import { CHATS_TAG, CUSTOM_SECTION_TAG_PREFIX, type SectionExpansionState } from "../../stores/room-list-v3/section";
import { MetaSpace } from "../../stores/spaces";
import { RoomNotificationStateStore } from "../../stores/notifications/RoomNotificationStateStore";
import { type RoomNotificationState } from "../../stores/notifications/RoomNotificationState";

vi.mock("../../utils/room/tagRoom", () => ({
    tagRoom: vi.fn(),
}));

vi.mock("../../utils/room/getSectionTagForRoom", () => ({
    getSectionTagForRoom: vi.fn().mockReturnValue(null),
}));

vi.mock("./utils", () => ({
    hasCreateRoomRights: vi.fn().mockReturnValue(false),
...
describe("RoomListViewModel", () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(SettingsStore, "setValue").mockImplementation(async (setting, roomId, level, value) => {
        ...
        vi.spyOn(sdkContext.roomListStore, "isLoadingRooms", "get").mockReturnValue(false);
        ...
        vi.mocked(hasCreateRoomRights).mockReturnValue(false);
    });
    ...
    describe("Room list updates", () => {
        it("should update room list when ListsUpdate event fires", () => {
            ...
            sdkContext.roomListStore.emit(RoomListStoreV3Event.ListsUpdate);
        ...
        it("should update loading state when ListsLoaded event fires", () => {
            vi.spyOn(sdkContext.roomListStore, "isLoadingRooms", "get").mockReturnValue(true);
            viewModel = new RoomListViewModel({
                client: matrixClient,
                spaceStore: sdkContext.spaceStore,
                roomViewStore: sdkContext.roomViewStore,
            ...
            sdkContext.roomListStore.emit(RoomListStoreV3Event.ListsLoaded);
    ...
    describe("Space switching", () => {
        it("should update room list when space changes", () => {
            ...
            vi.spyOn(sdkContext.spaceStore, "getLastSelectedRoomIdForSpace").mockReturnValue("!room1:server");
        ...
        it("should clear view models when space changes", () => {
            ...
            const disposeSpy1 = vi.spyOn(vm1, "dispose");
            const disposeSpy2 = vi.spyOn(vm2, "dispose");
    ...
    describe("Sticky room behavior", () => {
        ...
        it("should not apply sticky behavior when user changes rooms", async () => {
            ...
            vi.spyOn(sdkContext.roomViewStore, "getRoomId").mockReturnValue("!room3:server");
            dispatcher.dispatch({
                action: Action.ActiveRoomChanged,
    ...
    describe("Room item view models", () => {
        ...
        it("should dispose view models for rooms no longer visible", () => {
            ...
            const disposeSpy1 = vi.spyOn(vm1, "dispose");
            const disposeSpy3 = vi.spyOn(vm3, "dispose");
    ...
    describe("Room creation", () => {
        ...
        it("should dispatch CreateRoom action with parent space", () => {
            ...
            vi.spyOn(sdkContext.spaceStore, "activeSpaceRoom", "get").mockReturnValue(spaceRoom);
    ...
    describe("Keyboard navigation (ViewRoomDelta)", () => {
        ...
        it("should navigate to next room when delta is 1", async () => {
            ...
            vi.spyOn(sdkContext.roomViewStore, "getRoomId").mockReturnValue("!room1:server");
        ...
        it("should not navigate when current room is not found", async () => {
            ...
            vi.spyOn(sdkContext.roomViewStore, "getRoomId").mockReturnValue("!unknown:server");
    ...
    describe("Cleanup", () => {
        ...
        describe("Toast", () => {
            it("should show toast when SectionCreated event fires", () => {
                ...
                sdkContext.roomListStore.emit(RoomListStoreV3Event.SectionCreated);
            ...
            it("should show toast when RoomTagged event fires", () => {
                ...
                sdkContext.roomListStore.emit(RoomListStoreV3Event.RoomTagged);
        ...
        describe("Sections", () => {
            ...
            it("should compute activeRoomIndex relative to visible rooms when a section is collapsed", async () => {
                ...
                vi.spyOn(sdkContext.roomViewStore, "getRoomId").mockReturnValue("!reg1:server");
                dispatcher.dispatch({
                    action: Action.ActiveRoomChanged,
                    newRoomId: "!reg1:server",
                });

                // `waitFor` polls using real timers, which never fire while fake timers are
                // installed (and vitest fake timers aren't auto-detected by testing-library the
                // way jest's are). Use the fake-timer-aware flush helper instead.
            ...
            describe("custom section visibility by originating space", () => {
                ...
                beforeEach(() => {
                    vi.spyOn(sdkContext.spaceStore, "enabledMetaSpaces", "get").mockReturnValue([MetaSpace.Home]);
                    vi.spyOn(sdkContext.spaceStore, "spacePanelSpaces", "get").mockReturnValue([
            ...
            it("should apply sticky room within the correct section", async () => {
                ...
                vi.spyOn(sdkContext.roomViewStore, "getRoomId").mockReturnValue("!fav1:server");
                dispatcher.dispatch({
                    action: Action.ActiveRoomChanged,
            ...
            describe("Drag and drop", () => {
                ...
                it("should delegate changeSectionOrder to RoomListStoreV3.reorderSection", async () => {
                    const reorderSpy = vi
                        .spyOn(sdkContext.roomListStore, "reorderSection")
                ...
                it("should scroll the moved section back into view after reordering", async () => {
                    vi.spyOn(sdkContext.roomListStore, "reorderSection").mockResolvedValue(undefined);
    ...
    describe("changeRoomSection", () => {
        ...
        it("should call tagRoom with the room and target tag", () => {
            vi.spyOn(matrixClient, "getRoom").mockReturnValue(room1);
            vi.mocked(getSectionTagForRoom).mockReturnValue(null);
        ...
        it("should do nothing when the room is not found", () => {
            vi.spyOn(matrixClient, "getRoom").mockReturnValue(null);
        ...
        it("should do nothing when the room is already in the target section", () => {
            vi.spyOn(matrixClient, "getRoom").mockReturnValue(room1);
            vi.mocked(getSectionTagForRoom).mockReturnValue(DefaultTagID.Favourite);
