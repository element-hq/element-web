    ...
    implements RoomListItemViewActions
{
    ...
    public constructor(props: RoomItemProps) {
        ...
        if (this.dmUserId) {
            ...
            const userStatusWatchRef = SettingsStore.watchSetting("feature_user_status", null, () =>
                this.loadAndSetUserStatus(),
            );
            this.disposables.track(() => {
                SettingsStore.unwatchSetting(userStatusWatchRef);
            });
     ...
     */
    private loadAndSetUserStatus(): void {
        void (async () => {
