...
import * as mapUtilHooks from "../../../utils/location/useMap";
...
describe("<MBeaconBody />", () => {
    ...
    describe("latestLocationState", () => {
        ...
        it("opens maximised map view on click when beacon has a live location", async () => {
            const room = makeRoomWithStateEvents([aliceBeaconInfo], { roomId, mockClient });
            const beaconInstance = room.currentState.beacons.get(getBeaconInfoIdentifier(aliceBeaconInfo))!;
            beaconInstance.addLocations([location1]);
            const component = getComponent({ mxEvent: aliceBeaconInfo });
            ...
            await waitFor(() => {
                expect(component.container.querySelector(".mx_Map")).toBeInTheDocument();
            });
        ...
        it("updates latest location", async () => {
            const room = makeRoomWithStateEvents([aliceBeaconInfo], { roomId, mockClient });
    ...
    describe("redaction", () => {
        ...
        const mockGetRelationsForEvent = (locationEvents: MatrixEvent[] = []) => {
            ...
            vi.spyOn(relations, "getRelations").mockReturnValue(locationEvents);

            const getRelationsForEvent = vi.fn().mockReturnValue(relations);
        ...
        it("cleans up redaction listener on unmount", () => {
            ...
            const removeListenerSpy = vi.spyOn(beaconInfoEvent, "removeListener");
    ...
    describe("when map display is not configured", () => {
        beforeEach(() => {
            ...
            vi.spyOn(mapUtilHooks, "useMap").mockImplementation(({ onError }) => {
