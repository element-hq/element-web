...
import DialPad, { BUTTONS, BUTTON_LETTERS } from "./DialPad";

it("when hasDial is true, displays all expected numbers and letters", () => {
    render(<DialPad onDigitPress={vi.fn()} hasDial={true} onDialPress={vi.fn()} />);
...
it("clicking a digit button calls the correct function", async () => {
    const mockOnDigitPress = vi.fn();
    render(<DialPad onDigitPress={mockOnDigitPress} hasDial={true} onDialPress={vi.fn()} />);
...
it("clicking the dial button calls the correct function", async () => {
    const mockOnDial = vi.fn();
    render(<DialPad onDigitPress={vi.fn()} hasDial={true} onDialPress={mockOnDial} />);
