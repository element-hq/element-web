...
import { DEFAULT_DURATION_MS } from "./LiveDurationDropdown";
...
vi.mock("../../../utils/location/findMapStyleUrl", () => ({
    findMapStyleUrl: vi.fn().mockReturnValue("test"),
}));
...
vi.mock("../../../stores/OwnProfileStore", () => ({
    OwnProfileStore: {
        instance: {
            on: vi.fn(),
            displayName: "Ernie",
            getHttpAvatarUrl: vi.fn().mockReturnValue("image.com/img"),
        },
    },
}));
...
describe("<LocationShareMenu />", () => {
    ...
    const mockClient = getMockClientWithEventEmitter({
        ...
        unstable_setLiveBeacon: vi.fn().mockResolvedValue({ event_id: "1" }),
    ...
    beforeEach(async () => {
        ...
        vi.spyOn(MatrixClientPeg, "get").mockReturnValue(mockClient as unknown as MatrixClient);
    ...
    const setLocationGeolocate = () => {
        ...
        expect(vi.mocked(mockGeolocate.on)).toHaveBeenCalledWith("geolocate", expect.any(Function));
        const [, onGeolocateCallback] = vi
            .mocked(mockGeolocate.on)
    ...
    const setLocationClick = () => {
        // get the callback LocationShareMenu registered for geolocate
        expect(vi.mocked(mockMap.on)).toHaveBeenCalledWith("click", expect.any(Function));
        const [, onMapClickCallback] = vi.mocked(mockMap.on).mock.calls.find(([event]) => event === "click")!;
    ...
    describe("with live location disabled", () => {
        ...
        it("navigates to location picker when live share is enabled in settings store", () => {
            // @ts-ignore
            vi.mocked(SettingsStore.watchSetting).mockImplementation((featureName, roomId, callback) => {
    ...
    describe("Live location share", () => {
        ...
        it("opens error dialog when beacon creation fails", async () => {
            ...
            const logSpy = vi.spyOn(logger, "error").mockReturnValue(undefined);
            ...
            expect(vi.mocked(Modal).createDialog).toHaveBeenCalledWith(
                QuestionDialog,
                expect.objectContaining({
                    button: "Try again",
...
function enableSettings(settings: string[]) {
    vi.mocked(SettingsStore).getValue.mockReturnValue(false);
    vi.mocked(SettingsStore).getValue.mockImplementation((settingName: string): any => settings.includes(settingName));
}
