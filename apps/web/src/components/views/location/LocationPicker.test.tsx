...
import { findMapStyleUrl, LocationShareError } from "../../../utils/location";

vi.mock("maplibre-gl");

vi.mock("../../../utils/location/findMapStyleUrl", () => ({
    findMapStyleUrl: vi.fn().mockReturnValue("tileserver.com"),
}));
...
describe("LocationPicker", () => {
    describe("<LocationPicker />", () => {
        ...
        beforeEach(() => {
            ...
            vi.clearAllMocks();
            vi.mocked(mockMap).addControl.mockReset();
            vi.mocked(findMapStyleUrl).mockReturnValue("tileserver.com");
        });
        ...
        it("displays error when map emits an error", () => {
            ...
            act(() => {
                // @ts-ignore
                vi.mocked(mockMap).emit("error", { error: "Something went wrong" });
            });
        ...
        it("displays error when map display is not configured properly", () => {
            // suppress expected error log
            vi.spyOn(logger, "error").mockImplementation(() => {});
            vi.mocked(findMapStyleUrl).mockImplementation(() => {
        ...
        it("displays error when map setup throws", () => {
            ...
            // throw an error
            vi.mocked(mockMap).addControl.mockImplementation(() => {
        ...
        const testUserLocationShareTypes = (shareType: LocationShareType.Own | LocationShareType.Live) => {
            describe("user location behaviours", () => {
                ...
                it("sets position on geolocate event", () => {
                    ...
                    act(() => {
                        // @ts-ignore
                        vi.mocked(mockGeolocate).emit("geolocate", mockGeolocationPosition);
                    });
        ...
        describe("for Pin drop location share type", () => {
            ...
            it("does not set position on geolocate event", () => {
                vi.mocked(maplibregl.Marker).mockClear();
            ...
            it("sets position on click event", () => {
                ...
                act(() => {
                    // @ts-ignore
                    vi.mocked(mockMap).emit("click", mockClickEvent);
                });
