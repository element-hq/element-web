...
describe("<SmartMarker />", () => {
    ...
    it("updates marker position on change", () => {
        ...
        expect(maplibregl.Marker).toHaveBeenCalledTimes(1);
        // set positions
        expect(vi.mocked(mockMarker.setLngLat)).toHaveBeenCalledWith({ lat: 40, lon: 50 });
        expect(vi.mocked(mockMarker.setLngLat)).toHaveBeenCalledWith({ lat: 41, lon: 51 });
        expect(vi.mocked(mockMarker.setLngLat)).toHaveBeenCalledWith({ lat: 42, lon: 52 });
    });
