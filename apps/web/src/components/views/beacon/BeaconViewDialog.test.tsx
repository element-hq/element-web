...
import BeaconViewDialog from "./BeaconViewDialog";
...
describe("<BeaconViewDialog />", () => {
    ...
    const setupRoom = (stateEvents: MatrixEvent[] = []): Room => {
        ...
        vi.spyOn(room1, "getMember").mockReturnValue(aliceMember);
    ...
    beforeEach(() => {
        vi.spyOn(OwnBeaconStore.instance, "getLiveBeaconIds").mockRestore();
        vi.spyOn(OwnBeaconStore.instance, "getBeaconById").mockRestore();
        vi.spyOn(global.Date, "now").mockReturnValue(now);
        vi.clearAllMocks();
    });
    ...
    it("renders own beacon status when user is live sharing", () => {
        ...
        vi.spyOn(OwnBeaconStore.instance, "getLiveBeaconIds").mockReturnValue([beacon.identifier]);
        vi.spyOn(OwnBeaconStore.instance, "getBeaconById").mockReturnValue(beacon);
    ...
    it("renders map without markers when no live beacons remain", () => {
        ...
        vi.mocked(mockMap.fitBounds).mockClear();
    ...
    describe("focused beacons", () => {
        ...
        it("refocuses on same beacon when clicking list item again", () => {
            ...
            vi.spyOn(global.Date, "now").mockReturnValue(now + 1);
