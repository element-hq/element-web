...
import LeftPanelLiveShareWarning from "./LeftPanelLiveShareWarning";
import { OwnBeaconStore, OwnBeaconStoreEvent } from "../../../stores/OwnBeaconStore";
import dispatcher from "../../../dispatcher/dispatcher";
import { Action } from "../../../dispatcher/actions";

vi.mock("../../../stores/OwnBeaconStore", async () => {
    const EventEmitter = (await vi.importActual<typeof import("node:events")>("node:events")).EventEmitter;
    class MockOwnBeaconStore extends EventEmitter {
        public getLiveBeaconIdsWithLocationPublishError = vi.fn().mockReturnValue([]);
        public getBeaconById = vi.fn();
        public getLiveBeaconIds = vi.fn().mockReturnValue([]);
        public readonly beaconUpdateErrors = new Map<BeaconIdentifier, Error>();
        public readonly beacons = new Map<BeaconIdentifier, Beacon>();
    }
    return {
        // @ts-ignore
        ...(await vi.importActual("../../../stores/OwnBeaconStore")),
        OwnBeaconStore: {
...
describe("<LeftPanelLiveShareWarning />", () => {
    ...
    describe("when user has live location monitor", () => {
        beforeAll(() => {
            vi.mocked(OwnBeaconStore.instance).getBeaconById.mockImplementation((beaconId) => {
        ...
        beforeEach(() => {
            ...
            vi.mocked(OwnBeaconStore.instance).isMonitoringLiveLocation = true;
            vi.mocked(OwnBeaconStore.instance).getLiveBeaconIdsWithLocationPublishError.mockReturnValue([]);
            vi.mocked(OwnBeaconStore.instance).getLiveBeaconIds.mockReturnValue([
                beacon2.identifier,
                beacon1.identifier,
            ]);
        });
        ...
        afterAll(() => {
            vi.spyOn(document, "addEventListener").mockRestore();
        });
        ...
        it("removes itself when user stops having live beacons", async () => {
            ...
            act(() => {
                // @ts-ignore writing to readonly variable
                vi.mocked(OwnBeaconStore.instance).isMonitoringLiveLocation = false;
        ...
        it("refreshes beacon liveness monitors when pagevisibilty changes to visible", () => {
            ...
            const beacon1MonitorSpy = vi.spyOn(beacon1, "monitorLiveness");
            const beacon2MonitorSpy = vi.spyOn(beacon1, "monitorLiveness");

            vi.spyOn(document, "addEventListener").mockImplementation((_e, listener) =>
