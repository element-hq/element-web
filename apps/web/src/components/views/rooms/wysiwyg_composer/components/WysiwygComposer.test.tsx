...
import { parseEditorStateTransfer } from "../hooks/useInitialContent";
...
import * as Permalinks from "../../../../../utils/permalinks/Permalinks";
...
describe("WysiwygComposer", () => {
    const customRender = (onChange = vi.fn(), onSend = vi.fn(), disabled = false, initialContent?: string) => {
    ...
    describe("Mentions and commands", () => {
        ...
        beforeEach(async () => {
            ...
            vi.spyOn(Autocompleter.prototype, "getCompletions").mockResolvedValue([
                {
            ...
            vi.spyOn(Permalinks, "parsePermalink").mockReturnValue({
        ...
        describe("In message editing", () => {
            ...
            describe("Moving down", () => {
                ...
                it("Should close editing", async () => {
                    // When
                    vi.spyOn(EventUtils, "findEditableEvent").mockReturnValue(undefined);
                    const { textbox, spyDispatcher } = await setup(editorStateTransfer);
                    // Skipping the BR tag
                    const textNode = textbox.childNodes[textbox.childNodes.length - 2];
                    const { length } = textNode.textContent || "";
