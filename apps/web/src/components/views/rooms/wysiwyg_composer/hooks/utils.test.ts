...
import { handleClipboardEvent, isEventToHandleAsClipboardEvent } from "./utils";
import type { RoomUploadViewModel } from "../../../../../viewmodels/room/RoomUploadViewModel";

const mockUploadVM = {
    initiateViaDataTransfer: vi.fn().mockResolvedValue(undefined),
    initiateViaInputFiles: vi.fn().mockResolvedValue(undefined),
} as Partial<RoomUploadViewModel> as Mocked<RoomUploadViewModel>;
    ...
    it("calls the error handler when data types has text/html but data can not be parsed", () => {
        const originalEvent = createMockClipboardEvent({
            ...
            clipboardData: {
                files: [],
                types: ["text/html"],
                getData: vi.fn().mockReturnValue("<div>invalid html"),
            },
        });
        const output = handleClipboardEvent(originalEvent, originalEvent.clipboardData, mockUploadVM);
    ...
    it("calls fetch when data types has text/html and data can parsed", () => {
        const originalEvent = createMockClipboardEvent({
            ...
            clipboardData: {
                files: [],
                types: ["text/html"],
                getData: vi.fn().mockReturnValue(`<img src="blob:" />`),
            },
        });
        const output = handleClipboardEvent(originalEvent, originalEvent.clipboardData, mockUploadVM);
