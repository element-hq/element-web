...
describe("processMention", () => {
    ...
    it("can insert a mention into a text node", () => {
        ...
        const mockSetSuggestionData = vi.fn();
        const mockSetText = vi.fn();
        processMention(
            href,
            displayName,
...
describe("processSelectionChange", () => {
    ...
    it("returns early if current editorRef is null", () => {
        ...
        const nodeIteratorSpy = vi.spyOn(document, "createNodeIterator");

        processSelectionChange(mockEditorRef, vi.fn());
