...
describe("ManualDeviceKeyVerificationDialog", () => {
    ...
    function mockExistingDevices() {
        mockClient.getCrypto()!.getDeviceVerificationStatus = vi
            .fn()
            .mockImplementation(async (_userId, deviceId) =>
                deviceId === "DEVICEID" ? unverifiedDevice() : verifiedDevice(),
            );

        mockClient.getCrypto()!.getUserDeviceInfo = vi.fn().mockImplementation(async (userIds) => {
            const userDevices = new Map();
            userDevices.set("DEVICEID", { getFingerprint: vi.fn().mockReturnValue("FINGERPRINT") });
            userDevices.set("VERIFIED_DEVICEID", { getFingerprint: vi.fn().mockReturnValue("VERIFIED_FINGERPRINT") });
