...
import { fireEvent, render, screen, act } from "test-utils-rtl";
...
describe("InteractiveAuthDialog", function () {
    ...
    const mockClient = getMockClientWithEventEmitter({
        ...
        getFallbackAuthUrl: vi.fn().mockReturnValue(authUrl),
        getHomeserverUrl: vi.fn().mockReturnValue(homeserverUrl),
    });
    ...
    describe("SSO flow", () => {
        ...
        it("should complete an sso flow", async () => {
            vi.spyOn(global.window, "addEventListener");
            // @ts-ignore
            vi.spyOn(global.window, "open").mockImplementation(() => {});
            const onFinished = vi.fn();
            ...
            const onWindowReceiveMessageCall = vi
                .mocked(window.addEventListener)
                .mock.calls.find((args) => args[0] === "message");
