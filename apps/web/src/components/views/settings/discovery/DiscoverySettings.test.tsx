...
describe("DiscoverySettings", () => {
    ...
    it("displays alert if an identity server needs terms accepting", async () => {
        vi.mocked(client).getIdentityServerUrl.mockReturnValue("https://example.com");
        vi.mocked(client).getTerms.mockResolvedValue(sampleTerms);
    ...
    it("updates if ID server is changed", async () => {
        ...
        vi.mocked(client).getThreePids.mockClear();
    ...
    it("should not disable share button if terms accepted", async () => {
        vi.mocked(client).getThreePids.mockResolvedValue({
            threepids: [
                {
    ...
    it("should not show invalid terms", async () => {
        vi.mocked(client).getIdentityServerUrl.mockReturnValue("https://example.com");
        vi.mocked(client).getTerms.mockResolvedValue(invalidTerms);
