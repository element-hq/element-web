...
export interface MessagePanelMethods {
...
const useGhostReadMarkers = (
    readMarkerEventId: string | undefined | null,
    readMarkerVisible: boolean | undefined,
): {
    ghostReadMarkers: string[];
    collectGhostReadMarker: (node: HTMLElement | null) => void;
    onGhostTransitionEnd: (ev: TransitionEvent) => void;
} => {
    const [ghostReadMarkers, setGhostReadMarkers] = useState<string[]>([]);

    const previousReadMarker = useRef<{
        readMarkerEventId: string | undefined | null;
        readMarkerVisible: boolean | undefined;
    }>({
        readMarkerEventId,
    ...
    useEffect(() => {
        if (
            previousReadMarker.current.readMarkerVisible &&
            previousReadMarker.current.readMarkerEventId &&
            readMarkerEventId !== previousReadMarker.current.readMarkerEventId
        ) {
            setGhostReadMarkers((markers) => [...markers, previousReadMarker.current.readMarkerEventId!]);
            previousReadMarker.current = {
                readMarkerEventId,
                readMarkerVisible,
            };
        }
    }, [readMarkerEventId, readMarkerVisible]);

    const collectGhostReadMarker = useCallback((node: HTMLElement | null): void => {
        if (node) {
    ...
    const onGhostTransitionEnd = useCallback((ev: TransitionEvent): void => {
        // we can now clean up the ghost element
        const finishedEventId = (ev.target as HTMLElement).dataset.eventid;
        setGhostReadMarkers((markers) => markers.filter((eid) => eid !== finishedEventId));
    }, []);

    return { ghostReadMarkers, collectGhostReadMarker, onGhostTransitionEnd };
};
...
function getPendingEditItem(room: Room, timelineRenderingType: TimelineRenderingType): string | null {
    if (!room) {
        return null;
    }

    try {
        return localStorage.getItem(editorRoomKey(room.roomId, timelineRenderingType));
    } catch (err) {
        logger.error(err);
        return null;
    }
...
function isSentState(ev: MatrixEvent): boolean {
...
function getReadReceiptsForEvent(
    room: Room | undefined,
    event: MatrixEvent,
    threadId?: string,
): IReadReceiptProps[] | null {
    ...
    const receiptDestination = threadId ? room.getThread(threadId) : room;

    const receipts: IReadReceiptProps[] = [];

    if (!receiptDestination) {
        logger.debug("Discarding request, could not find the receiptDestination for event: " + threadId);
        return receipts;
    }
 ...
 */
export const MessagePanelNew: React.FC<IProps> = (props: IProps) => {
    ...
    // over all events.
    const readReceiptsByEvent = useRef<Map<string, IReadReceiptProps[]>>(new Map());
    ...
    const readReceiptsByUserId = useRef<Map<string, IReadReceiptForUser>>(new Map());

    // Cache these settings on mount since Settings is expensive to query,
    // and we check this in a hot code path. This is also cached in our
    // RoomContext, however we still need a fallback for roomless MessagePanels.
    const showHiddenEvents = useRef(SettingsStore.getValue("showHiddenEventsInTimeline"));

    const readMarkerNode = useRef<HTMLLIElement>(null);
    const whoIsTyping = useRef<WhoIsTypingTile>(null);
    const scrollPanel = useRef<ScrollPanel>(null);

    const eventTiles = useRef<Record<string, UnwrappedEventTile>>({});

    // A map to allow groupers to maintain consistent keys even if their first event is uprooted due to back-pagination.
    const grouperKeyMapRef = useRef(new WeakMap<MatrixEvent, string>());

    const { ghostReadMarkers, collectGhostReadMarker, onGhostTransitionEnd } = useGhostReadMarkers(
        props.readMarkerEventId,
        props.readMarkerVisible,
    );

    const showTypingNotifications = useSettingValue("showTypingNotifications");

    const shouldHideSender = useCallback((): boolean => {
        return !!props.room && props.room.getInvitedAndJoinedMemberCount() <= 2 && props.layout === Layout.Bubble;
    }, [props.room, props.layout]);

    const updateHideSender = useCallback((): void => {
        const newHideSender = shouldHideSender();
        setHideSender(newHideSender);
    }, [shouldHideSender]);

    // todo: passing this callback is correct?
    const [hideSender, setHideSender] = useState(shouldHideSender);

    useTypedEventEmitter(props.room, RoomStateEvent.Update, () => {
        updateHideSender();
    });
    ...
    useEffect(() => {
        updateHideSender();
    }, [props.layout, updateHideSender]);

    /* get the DOM node representing the given event */
    const getNodeForEventId = (eventId: string): HTMLElement | undefined => {
        if (!eventTiles.current) {
            return undefined;
        }

        return eventTiles.current[eventId]?.ref?.current ?? undefined;
    };
    ...
    useImperativeHandle(props.ref, () => {
        return {
            isAtBottom: () => scrollPanel.current?.isAtBottom(),
            getScrollState: () => scrollPanel.current?.getScrollState() ?? null,
            getReadMarkerPosition: (): number | null => {
                const readMarker = readMarkerNode.current;
                const messageWrapper = scrollPanel.current?.divScroll;
            ...
            scrollToTop: () => {
                scrollPanel.current?.scrollToTop();
            },
            scrollToBottom: () => {
                scrollPanel.current?.scrollToBottom();
            },
            handleScrollKey: (ev: React.KeyboardEvent | KeyboardEvent) => {
                scrollPanel.current?.handleScrollKey(ev);
            },
            scrollToEvent: (eventId: string, pixelOffset?: number, offsetBase?: number) => {
                scrollPanel.current?.scrollToToken(eventId, pixelOffset, offsetBase);
            },
            scrollToEventIfNeeded: (eventId: string) => {
                const node = getNodeForEventId(eventId);
                if (node) {
                    node.scrollIntoView({
                        block: "nearest",
                        behavior: "instant",
            ...
            updateTimelineMinHeight: (): void => {
                ...
                if (scrollPanelCurrent) {
                    const isAtBottom = scrollPanelCurrent.isAtBottom();
                    const whoIsTypingCurrent = whoIsTyping.current;
                    const isTypingVisible = whoIsTypingCurrent && whoIsTypingCurrent.isVisible();
    ...
    const getEventTiles = (): ReactNode[] => {
        ...
        const events: WrappedEvent[] = props.events.map((event) => {
            return { event, shouldShow: shouldShowEvent(event) };
        });
        ...
        for (let i = events.length - 1; i >= 0; i--) {
            ...
            if (!foundLastSuccessfulEvent && isSentState(event) && isEligibleForSpecialReceipt(event)) {
        ...
        readReceiptsByEvent.current = new Map();
        if (props.showReadReceipts) {
            readReceiptsByEvent.current = getReadReceiptsByShownEvent(events);
        }
        ...
        for (let i = 0; i < events.length; i++) {
            ...
            const { nextEventAndShouldShow, nextTile } = getNextEventInfo(events, i);
            ...
            if (!grouper) {
                if (shouldShow) {
                    ...
                    ret.push(
                        ...getTilesForEvent(prevEvent, wrappedEvent, last, false, nextEventAndShouldShow, nextTile),
                    );
                    prevEvent = event;
                }

                const readMarker = readMarkerForEvent(eventId, i >= lastShownNonLocalEchoIndex);
    ...
    const onTypingHidden = (): void => {
        ...
        if (scrollPanelCurrent) {
            ...
            scrollPanelCurrent.updatePreventShrinking();
    ...
    let whoIsTypingDom;
    if (props.room && showTypingNotifications && context.timelineRenderingType === TimelineRenderingType.Room) {
        whoIsTypingDom = (
            <WhoIsTypingTile room={props.room} onShown={onTypingShown} onHidden={onTypingHidden} ref={whoIsTyping} />
        );
    }

    let ircResizer: JSX.Element | undefined;
    if (props.layout == Layout.IRC) {
        ircResizer = <IRCTimelineProfileResizer minWidth={20} maxWidth={600} roomId={props.room?.roomId ?? null} />;
    }

    const classes = classNames(props.className, {
        mx_MessagePanel_narrow: context.narrow,
    });
    ...
    return (
        <ErrorBoundary>
            <ScrollPanel
                ...
                onFillRequest={props.onFillRequest}
                onUnfillRequest={props.onUnfillRequest}
                style={style}
