...
const distDtsPath = path.resolve(__dirname, "../dist/element-web-shared-components.d.ts");
...
function main() {
    ...
    // Also update dist/.d.ts if it exists (for faster dev workflow without full rebuild)
    if (fs.existsSync(distDtsPath)) {
        ...
        if (translationKeyRegex.test(distContent)) {
            ...
            console.log(`Updated TranslationKey in ${distDtsPath}`);
        }
    }
}
