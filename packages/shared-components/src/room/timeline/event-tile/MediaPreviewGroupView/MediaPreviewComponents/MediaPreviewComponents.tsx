...
export function TextContent(props: {
    ...
    return <div className={styles.textContent}>
        <Header {...props} />
        <Body {...props} />
...
export function Icon({ icon, iconOnClick, iconBg, iconFg, iconBorder }: { icon: JSX.Element, iconOnClick?: () => void, iconBg: string, iconFg: string, iconBorder: string }) {
    const style: CSSProperties = {
        border: `1px solid ${iconBorder}`,
        backgroundColor: iconBg,
    };

    icon = React.cloneElement(icon, { style: { color: iconFg } });


    if (iconOnClick) {
        return <div className={classNames(styles.icon, styles.iconClickable)} style={style}><button onClick={iconOnClick}>{icon}</button></div>
    } else {
        return <div className={styles.icon} style={style}>{icon}</div>
    }
}

export function Buttons({ buttons }: { buttons: Array<MediaPreviewEntryButton> }): JSX.Element {
    return <div className={styles.buttonGroup}>
        {buttons.map(({ icon, onClick }) => <button className={styles.button} onClick={onClick}>{icon}</button>)}
    </div>;
}
