...
test.describe("Invites", () => {
    ...
    test("should be able to reject an invite", async ({ page, homeserver, user, bot, app }) => {
        const roomId = await bot.createRoom({ is_direct: true });
        await bot.inviteUser(roomId, user.userId);
        await app.viewRoomByName("Bob");
    ...
    test(
        "should be able to reject an invite, report the room and ignore the user",
        { tag: "@screenshot" },
        async ({ page, homeserver, user, bot, app }) => {
            ...
            await expect(page.getByRole("dialog", { name: "Reject invitation" })).toMatchScreenshot(
